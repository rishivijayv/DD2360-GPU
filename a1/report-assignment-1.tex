...
Thus in all cases, not matter what execution time is considered for the GPU, there is a very noticeable speedup when the same problem size is executed on the GPU as opposed to on the CPU. The main reason for this is likely the fact that GPUs have a lot more simple cores that are able to handle large computations with a lot more light-weight threads as opposed to the CPU, where the cores are a lot more powerful and complex.
...
We performed this profiling on the GPU cluster offered by KTH, which runs the NVIDIA H100 80GB HBM3 GPU (as suggested by the \verb|nvidia-smi -q| command). According to the \href{https://www.techpowerup.com/gpu-specs/h100-sxm5-80-gb.c3900}{product specifications}, there are 16896 GPU cores available on the GPU. Additionally, according to the information from \verb|lscpu|, there are 112 CPU cores that can be used. So, there are $16896 / 112 = ~150$x more GPU cores when compared to the CPU cores. So, one might theoretically expect to see a 75x speed-up in the best case. 

We see that for \verb|hotspot3D|, we get much greater speed-ups than 150x in all cases, whereas for \verb|lavaMD|, we get less than 150x speedups. This might indicate a few things. Firstly, this re-affirms that GPU cores are different from CPU cores and thus a 1-1 comparison might not yield accurate results. GPU cores are built for throughput oriented and have specialized arithmetic units that greatly outnumber those present on the CPU. Thus, each of the GPU core is likely able to perform a lot more computation compared to the CPU counterpart, which would explain why there is a much higher than a simple 150x speedup for \verb|hostspot3D|. Secondly, the code for \verb|hotspot3D| might be much better suited for GPU optimization, which could explain why while there is still a speedup for \verb|lavaMD|, it is not as high as for \verb|hostpost3D|. 
