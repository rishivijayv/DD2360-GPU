...
void benchmark(float *a, int input_length) {
  ...
  auto ref_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
      ref_end - ref_start);
  ...
  auto cpy_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
      cpy_end - cpy_start);
  printf("\t...took: %ld ms\n\n", cpy_diff.count());
  ...
  int TPB = 1024;
  int NBlocks =
      1 + ((input_length - 1) / TPB); // assumes input_length != 0; computes
                                      // ceil(q) with q = x / y (int division).

  //@@ Launch the GPU Kernel here and add a timer and copy the GPU memory back
  // to the CPU here
  ...
  float naive_res = 0.0;
  cudaMemcpy(&naive_res, d_res, 1 * sizeof(float),
             cudaMemcpyDeviceToHost); // synchronises, no barrier needed afaik.

  auto naive_end = std::chrono::system_clock::now();
  auto naive_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
  ...
  cudaMemset(d_res, 0,
             1 * sizeof(float)); // Reset Memory, don't measure to not double
                                 // count (already incldued in initialisation).
  ...
  cudaMemcpy(&tiling_res, d_res, 1 * sizeof(float),
             cudaMemcpyDeviceToHost); // synchronises, no barrier needed afaik.

  auto tiling_end = std::chrono::system_clock::now();
  auto tiling_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
      tiling_end - tiling_start);
...
int main(int argc, char **argv) {
  ...
  if (argc == 2) {
    ...
    for (int i = 20; i < 31; i = i + 2) {
      input_length = static_cast<int>(pow(2, i));
      printf("\nInitialise:\n\tInput length: 2**%d = %d\n", i, input_length);
      float *a = nullptr;
      init_benchmark(&a, input_length);
      for (int k = 0; k < 3; ++k) {
