...
A remark about the initialization: by far the most time was spend with initializing the data on the CPU.
To reduce overhead, we reused the same initialized array for multiple benchmark runs to safe compute resources.
However, for each run, fresh GPU memory was allocated and freed.

We decided to use much larger problem sizes than suggested in the exercise, mainly because for small input sizes runtimes were basically instantaneous.
To get any meaningful resutls, we benchmarked at array sizes $2^{20}, 2^{22}, 2^{24}, ..., 2^{30}$.

\subsection{Na\"ive GPU implementation}\label{sec:2_naive}
\begin{figure}[t]
  \begin{center}
    \includegraphics[width=.75\textwidth]{../images/q2/runtimes_colab.png}
  \end{center}
  \caption{}\label{fig:runtimes_ex2}
\end{figure}
...
Runtime did in fact decrease slightly for large enough inputs (see figure \ref{fig:runtimes_ex2}), however, not in relation to the degree of parallization.
Besides bad performance, we also ran into precision errors for large inputs (see section \ref{sec:2_init}), ergo the computed solution is incorrect (without aggregating using a double data type).

\subsubsection{Analysis}
\begin{itemize}
  \item Global memory reads: $N$ (one for each element)
  \item Atomic operations: $N$
  \item Global memory: $N$ * 4 bytes (input array, floats) + 4 bytes (aggregated value, 1 float)
  \item Shared mempory: No shared memory (on block level)
\end{itemize}
...
Each thread block has a shared memory variable, each thread accumulates one value onto this variable.
Afterwards, one thread per threadblock aggregates the shared value to the global solution.

In figure \ref{fig:runtimes_ex2}, the purple line represents this tiling on block level approach.
As we can see, it outperforms the na"ive (red line) as well as the cpu reference solution (orange line) on larger input sizes.
Besides runtime optimizations, this also solves precision problem.
Rounding erros happen, when very small values are added on very large ones.
In the na"ive solution, all values are aggregated sequentially, therefore after enough iterations, the aggregated value becomes big enough so that information is lost on each addition.
By using the tiling apprach, each block aggregates values within each precision limits.
In the second step, all block level values are again within a similar dimension, increasing the numerical stability of the algorithm.

\subsubsection{Analysis}
\begin{itemize}
  \item Global memory reads: $N$ (one for each element)
  \item Atomic operations: $N$ (on shared level) + $N$/1024 (global, $N$/1024 = number of blocks)
  \item Global memory: $N$ * 4 bytes (input array, floats) + 4 bytes (aggregated value, 1 float)
  \item Shared mempory: $N$/1024 * 4bytes (one aggregation value per block)
\end{itemize}
...
However, it performs still below expectation.
...
Idea: each thread does not add one element of the big array onto an accumulator, but independently accumulates $NT$ elements.
The thread can use a local register for this, which is a lot more efficient than shared or global memory.
Then, results from each thread are accumulated on block level, then globally.
Note that $NT$ is a paramter which can be optimised.
For this assignement we tried different values and found that values in the dimension of $NT \in \{1024, 2048, 4096\}$ worked the best and we used $NT = 1024$ for the rest of this exercise.

The performance increase was enormous. We achieved runtimes smaller than 1ms for all but the largest input size.
For the biggest input, we performed still under 3ms. This is over an $1250\times$ performance increase!

\subsubsection{Analysis}
\begin{itemize}
  \item Global memory reads: $N$ (one for each element)
  \item Atomic operations: $N$/1024/1024 (shared, $N$/1024/1024 = number of blocks)
  \item Global memory: $N$ * 4 bytes (input array, floats) + 4 bytes (aggregated value, 1 float)
  \item Shared mempory: $N$/1024/1024 * 4bytes (one aggregation value per block)
\end{itemize}

\textbf{Shared Memory Configuration Size and Achieved Occupancy:}\\
Section \ref{sec:ncu} shows in detail the profiling information gained by \verb|ncu|.
I want to highlight the \verb|Achieved Occupancy|, where we achieved $>99\%$.
This is basically optimal, therefor not really much to improve in this regard.\\
\verb|Shared Memory Configuration Size| indicates the shared memory size, in bytes, that is configured by the CUDA driver for this kernel launch, per block\footnote{\url{https://forums.developer.nvidia.com/t/meaning-of-shared-memory-configuration-size/199669/2}}.
The allocated value per block is $32KB$, which is around the size of 8000 floats.
This memory can be filled and accessed by all threads within the threadblock.
Further optimizations could investigate if thread access patterns could be changed so that memory bursts can be used even more effective and elapsed cycles could be further improved.
...
\section{Appendix}
\subsection{Runtime Specs}\label{sec:specs}
We used the Google Colab runtime environment, which at the time of testing provided us the following hardware:
\subsubsection{CPU specs}\label{sec:cpu_specs}
\begin{verbatim}
Architecture:                x86_64
...
\subsubsection{GPU specs}\label{sec:gpu_specs}
\begin{verbatim}
...
\subsection{Additional Information Exercise 2}\label{sec:app:ex2}
\subsubsection{NCU Output}\label{sec:ncu}
\begin{verbatim}
Initialise:
	Input length: 2**30 = 1073741824
	...took: 54755 ms

==PROF== Connected to process 6781 (/content/reduction)
	...took: 1561437401 ns

==PROF== Profiling "reduction_kernel_tiling_mem_adj" - 0: \
         0%....50%....100% - 9 passes
	...took: 688491412 ns

==PROF== Disconnected from process 6781
[6781] reduction@127.0.0.1
  reduction_kernel_tiling_mem_adj(float *, float *, int, int) \
  (1024, 1, 1)x(1024, 1, 1), Context 1, Stream 7, Device 0, CC 7.5
    ...
    Elapsed Cycles                cycle    3,730,476
    Memory Throughput                 %        75.98
    DRAM Throughput                   %         0.39
    Duration                         ms         6.38
    L1/TEX Cache Throughput           %        94.93
    L2 Cache Throughput               %         0.15
    SM Active Cycles              cycle 3,649,244.88
    Compute (SM) Throughput           %        75.98
    ----------------------- ----------- ------------

    INF   Compute and Memory are well-balanced: To reduce runtime, \
          both computation and memory traffic must be reduced. 
          Check both the Compute Workload Analysis and Memory \
    ...
    Static Shared Memory Per Block        byte/block               4
    # SMs                                         SM              40
    ...
    Metric Name                Metric Unit Metric Value
    -------------------------- ----------- ------------
    Average DRAM Active Cycles       cycle   124,362.50
    Total DRAM Elapsed Cycles        cycle  254,738,432
    Average L1 Active Cycles         cycle 3,649,244.88
    Total L1 Elapsed Cycles          cycle  149,156,192
    Average L2 Active Cycles         cycle   280,243.66
    Total L2 Elapsed Cycles          cycle  174,470,752
    Average SM Active Cycles         cycle 3,649,244.88
    Total SM Elapsed Cycles          cycle  149,156,192
    Average SMSP Active Cycles       cycle 3,649,189.76
    Total SMSP Elapsed Cycles        cycle  596,624,768
    -------------------------- ----------- ------------
\end{verbatim}
