...
For the streamed implementation, we first determine the number of segments the original array will be divided in to based on the provided \verb|S_seg|. We then create 4 streams as requested by the assignment, and then iterate through the segments, distributing them to the 4 streams in a \textbf{round-robin} fashion (segment \verb|i| goes to stream \verb|i \% 4|), taking care of appropriate boundary conditions. We use \verb|cudaMemcpyAsync| for D-to-H and H-to-D memory transfer. For the kernel launch configuration, we use 1024 threads per block, and the number of blocks that the kernel acts on is $\ceil{S_{seg} / 1024}$ (except for the last segment which might include less than $S_{seg}$ elements. 
...
\subsection{Profiling Using nvprof}
We ran \verb|nvprof| on Google Colab with the Tesla T4 GPUs. The \verb|q2/colab.ipynb| file contains the short Jupyter Notebook containing information about the \verb|nvprof| run. The file obtained from the \verb|nvprof| run is available in the \verb|q2/| directory with the name \verb|vector_addition_streams.nvvp|.
Unfortunately, we had a lot of issues installing the NVIDIA Visual Profiler on an M1 Macbook. We made a Canvas post about it \href{https://canvas.kth.se/courses/57317/discussion_topics/513951}{here}. We tried a few versions but faced the same problem. As a workaround, we even tried to install an Ubuntu VM using UTM on the MacOS and tried to get the NVIDIA CUDA Toolkit, but there we also faced issues with the toolkit and some of the tools not installing properly. When we were finally able to install NVVP on a MacOS laptop, the CUDA versions seem to be an issue. The Visual Profiler is available for MacOS up to CUDA 12.4, but the CUDA \verb|nvprof| version used on Google Colab seems to now be CUDA 12.5. Because of this, we got \textit{Unsupported File Type} errors when trying to visualize the profiling file in NVVP v12.4. Finally, trying to use the School GPU Cluster did not work either, since even though \verb|nvprof| version there is 12.4, using \verb|nvprof| gives us a \textit{nvprof is not supported on devices with compute capability 8.0 and higher} error.

Because of this, unfortunately we are not able to present a visualization of the profiling file we obtained from \verb|nvprof|. However, the file profiled using is included with our assignment submission, and given the runtimes and speedup we observed in Section \ref{sec:q2_part1}, we believe that there is a computation-communication overlap because of our usage of streams. We believe that visualizing this file NVVP from CUDA 12.5 should show this overlap. 
