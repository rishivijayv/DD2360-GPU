...
\newcommand{\itodo}[2][yellow]{\vspace{0.5em}\todo[inline, color=#1!40]{#2}}
...
The code for this part of the assignment can be found in the \href{https://github.com/rishivijayv/DD2360-GPU/tree/main/a3/bonus}{GitHub repository}.
For rerunning the experiment, you may want to use the \href{https://colab.research.google.com/github/rishivijayv/DD2360-GPU/blob/main/a3/bonus/colab.ipynb}{Google Colab Notebook}.

\subsection{Changes to the Makefile}
We only changed the Makefile in respect to two things (see~\ref{lst:bonus3}):
first, we changed the architecture flag to \verb|sm_75|.
Second, we added two targets to the Makefile, cpu and gpu, with cpu being the default.
The only change to the original Makefile is that it passes \verb|-DUSE_CPU| or \verb|-DUSE_GPU| to the compiler.
This sets a pre-processer definition, which then either compiles the CPU or GPU version (via \verb|#ifdef|).
That made compiling the different versions a bit easier.
...
Additionally, within the \verb|mover_PC| method, the electric fields are \textbf{read-only}.
...
We ran both version for 10 iterations and averaged over three runs.
The output of the simulation provides the average time spent in both the \verb|mover_PC| and \verb|interpolate| method.

\begin{table}[ht]
  \caption{Average runtimes CPU vs GPU}\label{tab:bonus1}
  \begin{center}
    \begin{tabular}[c]{|l|c|c|}
      ...
      \hline
      \textbf{Tot. Simulation Time (s)} & 70.32 & 33.48\\
      \textbf{Mover Time / Cycle   (s)} & 3.67 & 0.27\\
      \textbf{Interp. Time / Cycle (s)} & 2.87 & 2.76\\
      \hline
    \end{tabular}
  \end{center}
\end{table}

Table~\ref{tab:bonus1} demonstrates the runtime improvements for the GPU implementation compared to the CPU implementation.
It improves the runtime for the \verb|mover_PC| method by factor 10.

The goal of this task was to get a basic implementation running.
We preserve further profiling and performance improvements for the project.
Following ideas would be interesting to investigate:
\begin{itemize}
  \item Low hanging fruit: move the GPU memory allocation outside the \verb|mover_PC| method.
    At the moment the allocation happens every cycle for every species.
  \item Another idea could be to use pinned memory to decrease copy operations during \verb|cudaMemcpy|.
  \item Possibly big performance gains could be achieved by using streams to overlap computation and memory transfer for different species.
\end{itemize}

\section{Appendix}
\subsection{Bonus listings}

\begin{lstlisting}[language=c++, caption=Changes to Makefile, label=lst:bonus3]
...
ARCH=sm_75
...
...
...
$(BIN)/$(TARGET): $(OBJS)
	$(NVCC) $(NVCCFLAGS) -D$(DEVICE) $+ -o $@

$(SRCDIR)/%.o: $(SRCDIR)/%.cu
	$(NVCC) $(NVCCFLAGS) -D$(DEVICE) $< -c -o $@

$(OBJDIR)/%.o: $(SRCDIR)/%.cpp
	[ -d $(OBJDIR) ] || mkdir $(OBJDIR)
	$(NVCC) $(CXXFLAGS) -D$(DEVICE) $< -c -o $@
...
\end{lstlisting}
